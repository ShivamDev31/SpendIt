...
inline fun <reified T> DocumentReference.getObservable(): Observable<T> = Observable.create { emitter ->
...
inline fun <reified T> CollectionReference.getObservable(): Observable<List<T>> = Observable
        .create { emitter ->
...
inline fun <reified T> Query.getObservable(): Observable<List<T>> = Observable
        .create { emitter ->
            val listener = addSnapshotListener { querySnapshot, firebaseFirestoreException ->
                querySnapshot?.let {
                    try {
...
fun <T : Any> DocumentReference.mergeDocument(item: T): Completable = Completable
        .create { emitter ->
...
fun <T : Any> Task<T>.getCompletable(): Completable = Completable
        .create { emitter ->
