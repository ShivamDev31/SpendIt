...
/**
 * Set's the parameter [item] at the given [DocumentReference] (receiver)
 * and merge with the previous data if the document already exists and returns a
 * completable that completes when the transaction is complete or calls onError otherwise.
 *
 * @param item to be set at the [DocumentReference]
 * @receiver [DocumentReference] to set the item to
...
/**
 * Decrement's a value at the given [DocumentReference] (receiver) by [decrement] and returns a
 * Single that emits the new value if the [Transaction] completes successfully or calls onError
 * otherwise
 *
 * @param fieldName of the field to be incremented
 ...
 * @receiver [DocumentReference] to update
 */
fun DocumentReference.decrementField(fieldName: String, decrement: Long = 1): Single<Long> {
    return FirebaseFirestore.getInstance()
            .runTransactionSingle(Transaction.Function {
                val docSnapshot = it.get(this)
                val newValue = docSnapshot.getLong(fieldName) + decrement
                it.update(this, fieldName, newValue)
                newValue
            })
}